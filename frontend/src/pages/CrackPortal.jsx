...
import { Lock, Unlock, KeyRound, ShoppingCart, X, CheckCircle2, Search, Download } from 'lucide-react';
...
export default function CrackPortal() {
  ...
  const handleReveal = (gameId) => {
    setRevealedGames((prev) => ({ ...prev, [gameId]: true }));
  };

  const baseGames = games.filter((game) => game.game_type === 'crack');
            ...
            <div key={game.id} className="glass-panel fade-in" style={{ borderColor: 'var(--steam-green)', display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '1.5rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                ...
                {!revealedGames[game.id] ? (
                  ...
                  <div className="fade-in" style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    {game.download_link ? (
                      <a href={game.download_link} target="_blank" rel="noreferrer" className="btn btn-primary" style={{ justifyContent: 'center', fontWeight: 'bold' }}>
                        <Download size={18} /> Click để tải xuống
                      </a>
                    ) : (
                      <div style={{ color: '#ffbaba' }}>Admin chưa cập nhật link tải. Vui lòng liên hệ hỗ trợ.</div>
                    )}
                  </div>
                )}
              </div>
            ...
            <div style={{ padding: '0 1.5rem 1.5rem 1.5rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 'auto' }}>
                <div>
                  {game.discount ? (
                    <>
                      <div style={{ fontSize: '0.8rem', color: '#aaa', textDecoration: 'line-through' }}>{formatPrice(game.price)}</div>
                      <div style={{ color: 'var(--steam-green)', fontWeight: 'bold', fontSize: '1.2rem' }}>{formatPrice(game.discount)}</div>
                    </>
                  ) : (
                    <div style={{ color: 'var(--steam-green)', fontWeight: 'bold', fontSize: '1.2rem' }}>{formatPrice(game.price) || 'Liên hệ'}</div>
                  )}
                </div>
      ...
      {successOrderInfo && (
        ...
        >
          <div className="glass-panel fade-in" style={{ width: '90%', maxWidth: '400px', background: 'var(--steam-bg-darker)', position: 'relative', textAlign: 'center' }}>
            ...
            <p style={{ color: '#ccc', marginBottom: '0.5rem' }}>Mã đơn hàng của bạn:</p>
